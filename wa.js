...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .nav a{color:#d6dee8;text-decoration:none;padding:11px 12px;border-radius:12px;font-size:.86rem;cursor:pointer}
...
<div class="shell">
  <aside class="sidebar">
    ...
    <nav class="nav">
      <a class="active" data-nav="overview">Overview</a>
      <a data-nav="analytics">Analitik</a>
      <a data-nav="transactions">Transaksi</a>
      <a data-nav="budget">Budget</a>
      <a id="admin-nav" data-nav="admin" style="display:none">Admin</a>
      <a data-nav="system">Sistem</a>
      <a data-nav="settings">Pengaturan</a>
    </nav>
    <div class="side-meta">
...
<script>
  ...
  let activeView = localStorage.getItem("dashboardActiveView") || "overview";
  ...
  function readUiSettings() {
    try {
      return { ...defaultUiSettings, ...JSON.parse(localStorage.getItem("dashboardUiSettings") || "{}") };
    } catch {
  ...
  function applyUiSettings() {
    ...
    localStorage.setItem("dashboardUiSettings", JSON.stringify(uiSettings));
    scheduleAutoRefresh();
  }
  ...
  function showView(view) {
    const allowedViews = Array.from(document.querySelectorAll("[data-nav]")).map(link => link.dataset.nav);
    activeView = allowedViews.includes(view) ? view : "overview";
    ...
    document.querySelectorAll("[data-view]").forEach(el => {
      el.hidden = !String(el.dataset.view || "").split(/\s+/).includes(activeView);
    });
