...
(function(){
  ...
  async function loadDashboard(silent){ if(!token){ openLock(); return; } try{ if(!silent) showToast("Memuat data realtime..."); var q=queryString(); var res=await fetch("/api/dashboard" + (q ? "?"+q : ""), {headers:apiHeaders()}); var data=await res.json(); if(!res.ok) throw new Error(data.message || "Akses dashboard tidak valid"); current=data; sessionStorage.setItem("dashboardAccess", token); openApp(); userOptions(data); periodOptions(data); var f=data.finance || {}; if(!f.available){ txt("hero-title","Data belum tersedia"); txt("hero-copy",f.message || "Belum ada data keuangan."); return; } renderMetrics(data); renderWallets(f); renderBinance(f.binance || (data.integrations && data.integrations.binance)); renderCategories(f); renderBudgets(f); renderTips(f); renderCommands(data); renderIntegrations(data); populateCategories(); renderTransactions(); drawTrend(f); sessionStorage.setItem("dashboardPeriod", selectedPeriod || ""); sessionStorage.setItem("dashboardNumber", selectedNumber || ""); }catch(e){ openLock(); showToast(e.message || "Dashboard gagal dimuat"); } }
