...
function normalisasiNomorWhatsApp(value) {
    const userPart = String(value || "").split("@")[0].split(":")[0];
    return userPart.replace(/\D/g, "");
}
const ambilNomorDariJid = jid => normalisasiNomorWhatsApp(jid);
...
function nomorAdalahSuperAdmin(nomor) {
    return SUPER_ADMIN_NUMBERS.includes(normalisasiNomorWhatsApp(nomor));
}
...
function buatTokenAksesDashboard(nomor, role = "user") {
    const cleanNumber = normalisasiNomorWhatsApp(nomor);
...
function verifikasiTokenAksesDashboard(token) {
    try {
        ...
        const number = normalisasiNomorWhatsApp(data.number);
...
function nomorTargetDashboard(akses, urlObj) {
    const diminta = normalisasiNomorWhatsApp(urlObj.searchParams.get("nomor") || "");
...
function cariSheetByNomorLoose(doc, nomor) {
    const exact = doc.sheetsByTitle[nomor];
    if (exact) return exact;
    const candidates = Object.values(doc.sheetsByTitle || {}).filter(sheet => {
        const title = normalisasiNomorWhatsApp(sheet.title);
        return title.length >= 10 && nomor.startsWith(title) && nomor.length - title.length <= 3;
    });
    return candidates.length === 1 ? candidates[0] : null;
}

async function getSheetByNomor(jid, { createIfMissing = true, looseMatch = true } = {}) {
    const nomor = ambilNomorDariJid(jid);
    ...
    let sheet = looseMatch ? cariSheetByNomorLoose(doc, nomor) : doc.sheetsByTitle[nomor];
    if (!sheet) {
    ...
    const cacheValue = { sheet, at: Date.now() };
    cacheSheet[nomor] = cacheValue;
    const canonicalTitle = normalisasiNomorWhatsApp(sheet.title);
    if (canonicalTitle) cacheSheet[canonicalTitle] = cacheValue;
    return sheet;
}
...
// ── LAPORAN KEUANGAN ──────────────────────────────────────────
function laporanKeuanganKosong(meta = {}) {
    return { totalMasuk:0, totalKeluar:0, saldo:0, detailKategori:{}, saldoDompet:{}, trenHarian:{}, transaksi:[], rows:[], ...meta };
}

async function buatLaporanKeuangan(tipe, jid, options = {}) {
    const sheet = await getSheetByNomor(jid, options);
    if (!sheet) return laporanKeuanganKosong({ missingSheet:true });
...
// ── TREN HARIAN ───────────────────────────────────────────────
async function buatTrenHarian(jid, hari=7, options = {}) {
    const sheet = await getSheetByNomor(jid, options);
    if (!sheet) return [];
    const rows  = await sheet.getRows();
    const sekarang = sekarangWita();
...
async function buatDataDashboardWeb(akses, nomorDipilih = "") {
    ...
    const nomorDiminta = normalisasiNomorWhatsApp(nomorDipilih || "");
    ...
    try {
        const [lapHari, lapBulan, lapSemua, tren] = await Promise.all([
            buatLaporanKeuangan("hari", ownerJid, { createIfMissing:false }),
            buatLaporanKeuangan("bulan", ownerJid, { createIfMissing:false }),
            buatLaporanKeuangan("semua", ownerJid, { createIfMissing:false }),
            buatTrenHarian(ownerJid, 14, { createIfMissing:false })
        ]);
        const budget = getBudget(ownerJid);
        if (lapSemua.missingSheet) {
            base.finance = {
                available: false,
                period: bulan,
                owner: maskNomor(nomorAktif),
                message: `Sheet transaksi untuk nomor ${maskNomor(nomorAktif)} belum ditemukan. Minta link dashboard baru dari bot dengan command dashboard web, lalu pastikan nomor ini sudah pernah mencatat transaksi.`
            };
            return base;
        }
