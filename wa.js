...
function buatTabelWhatsapp(columns, rows, { title = "", maxRows = 12, emptyText = "Tidak ada data." } = {}) {
    ...
    const widths = columns.map(col => {
        ...
        const contentWidth = Math.max(minWidth, ...data.map(row => String(row[col.key] ?? "").length));
    ...
    const more = rows.length > maxRows ? `\n_+${rows.length - maxRows} baris lain tidak ditampilkan._` : "";
...
async function buatLaporanTabel(tipe, jid) {
    ...
    teks += "\n\n" + buatTabelWhatsapp([
        { key:"tgl", label:"Tgl", width:5 },
        { key:"tipe", label:"+/-", width:3 },
        { key:"ket", label:"Keterangan", width:18 },
        { key:"rp", label:"Rp", width:12, align:"right" }
    ], transaksiRows, { title:"Transaksi Terbaru", emptyText:"Belum ada transaksi pada periode ini." });

    teks += `\n\n💡 Ketik *dashboard* untuk ringkasan pintar atau *analisis* untuk saran AI.`;
    return teks;
}
...
async function buatDashboardKeuangan(jid) {
    ...
    const budgetRows = Object.entries(budget)
        ...
        .filter(row => row.pakai !== "0" || Number(row.persen.replace("%","")) >= 75)
    ...
    ], budgetRows, { title:"Budget yang Perlu Dipantau", emptyText:"Belum ada budget yang terpakai atau mendekati limit." });
...
async function eksporLaporan(tipe, jid) {
    const now  = new Date().toLocaleString("id-ID",{timeZone:APP_TIMEZONE});
    const tabel = await buatLaporanTabel(tipe, jid);
    return `${tabel}\n\n📤 *Export dibuat:* ${now}\n_Laporan ini dibuat otomatis oleh Bot Keuangan WA._`;
}
  ...
  <section class="grid">
    <div class="panel">
      ...
      <div class="feature-list">
        ...
        <div class="feature"><b>Laporan tabel</b><span>Ringkasan, kategori, dompet, dan transaksi terbaru dalam format rapi.</span></div>
