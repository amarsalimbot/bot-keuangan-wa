...
async function eksporLaporan(tipe, jid, opsi = {}) {
    ...
    const caption =
`📤 *EXPORT LAPORAN ${label.toUpperCase()}*
...
async function buatDataDashboardWeb(akses, nomorDipilih = "", periodeDipilih = "") {
    ...
    const base = {
        ...
        commands: [
            ...
            { cmd:"export Mei 2026", desc:"Unduh CSV periode historis" }
        ]
    };
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  :root{
    --bg:#eef2f4;--paper:#ffffff;--paper-soft:#f7f9fa;--line:#d9e1e6;--text:#17212b;--muted:#667581;
    --green:#0a8a62;--green-soft:#e5f6ef;--red:#c3484f;--red-soft:#fff0f1;--blue:#255fbd;--blue-soft:#edf4ff;
    --amber:#b67810;--amber-soft:#fff5dc;--teal:#087f85;--ink:#111b24;--shadow:0 12px 32px rgba(26,42,52,.09);
  }
  body{font-family:Inter,'Segoe UI',system-ui,sans-serif;background:linear-gradient(180deg,#f7f9fa 0,#edf2f4 100%);color:var(--text);min-height:100vh}
  ...
  .sidebar{background:var(--ink);border-right:1px solid #24313b;color:#f7fafc;padding:20px 16px;display:flex;flex-direction:column;gap:18px;position:sticky;top:0;height:100vh}
  .brand{display:flex;align-items:center;gap:12px;padding:6px 4px}
  .mark{width:42px;height:42px;border-radius:8px;background:#14a777;color:white;display:grid;place-items:center;font-weight:900;box-shadow:0 8px 22px rgba(20,167,119,.25)}
  .brand h1{font-size:1rem;line-height:1.2}.brand small{display:block;color:#9cafb9;font-size:.76rem;margin-top:3px}
  .nav{display:grid;gap:6px}
  .nav-item{width:100%;border:1px solid transparent;background:transparent;color:#c7d2d9;border-radius:8px;padding:10px 11px;display:flex;align-items:center;gap:10px;cursor:pointer;text-align:left;font-weight:800;font-size:.86rem}
  .nav-item:hover{background:#1b2a34;color:#fff}.nav-item.active{background:#203a38;border-color:#2c5750;color:#fff}
  .nav-swatch{width:8px;height:8px;border-radius:3px;background:var(--blue);flex:none}.nav-item[data-view="analytics"] .nav-swatch{background:#42b8a0}.nav-item[data-view="reports"] .nav-swatch{background:#e5b454}.nav-item[data-view="trend"] .nav-swatch{background:#67a9e8}.nav-item[data-view="budget"] .nav-swatch{background:var(--amber)}.nav-item[data-view="transactions"] .nav-swatch{background:var(--green)}.nav-item[data-view="catalog"] .nav-swatch{background:#b76ca5}.nav-item[data-view="system"] .nav-swatch{background:var(--teal)}.nav-item[data-view="admin"] .nav-swatch{background:var(--red)}
  .side-meta{margin-top:auto;display:grid;gap:8px;padding:12px;border:1px solid #293a45;border-radius:8px;background:#17242d;font-size:.78rem;color:#9cafb9}
  .side-meta b{color:#f4f8fa}
  .content{padding:24px;display:grid;align-content:start;gap:18px;max-width:1440px;width:100%}
  ...
  .metric-card,.panel-card{background:var(--paper);border:1px solid var(--line);border-radius:8px;box-shadow:var(--shadow)}.mini{background:var(--paper-soft);border:1px solid var(--line);border-radius:8px}
  ...
  .finance-hero{background:#132b2b;color:#f6fbfa;border:1px solid #234b47;border-radius:8px;padding:20px;display:grid;grid-template-columns:minmax(0,1.3fr) repeat(3,minmax(135px,.5fr));gap:14px;align-items:center;box-shadow:0 18px 42px rgba(18,43,43,.18)}
  .hero-copy h3{font-size:1.18rem}.hero-copy p{color:#b7cbc8;font-size:.84rem;margin-top:6px;max-width:560px}.hero-stat{border-left:1px solid #315450;padding-left:14px}.hero-stat span{display:block;color:#a8c0bc;font-size:.74rem}.hero-stat b{display:block;font-size:1.08rem;margin-top:5px}.score{color:#52d0a4}
  .insight-list{display:grid;gap:8px}.insight-item{display:flex;gap:10px;align-items:flex-start;padding:11px;border:1px solid var(--line);border-radius:8px;background:#fff}.insight-mark{width:8px;height:8px;border-radius:3px;background:var(--green);margin-top:5px;flex:none}
  .catalog-toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.catalog-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:10px}.category-card{border:1px solid var(--line);border-radius:8px;background:#fff;padding:13px;display:grid;gap:8px}.category-top{display:flex;gap:9px;align-items:center}.category-swatch{width:12px;height:34px;border-radius:4px;flex:none}.category-card h4{font-size:.9rem}.category-card p{font-size:.76rem;color:var(--muted);line-height:1.45}.category-meta{display:flex;gap:6px;flex-wrap:wrap}.tag{display:inline-flex;border:1px solid var(--line);border-radius:999px;padding:4px 7px;font-size:.7rem;color:var(--muted);background:#fff}
  .budget-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.budget-card-list{display:grid;gap:9px}.budget-card{display:grid;gap:7px;padding:12px;border:1px solid var(--line);border-radius:8px;background:#fff}.budget-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:8px}.budget-card-head button{border:0;background:transparent;color:var(--blue);font-weight:900;cursor:pointer}.budget-card .bar-meta{align-items:center}
  .smart-suggestion{display:none;border:1px solid #b9ded1;background:var(--green-soft);border-radius:8px;padding:9px 10px;font-size:.78rem;color:#155c47}.smart-suggestion.show{display:flex;align-items:center;justify-content:space-between;gap:8px}.smart-suggestion button{border:0;background:#fff;border-radius:6px;padding:5px 8px;color:#155c47;font-weight:900;cursor:pointer}
  ...
  .report-banner{background:#172a36;color:#fff;border:1px solid #294352;border-radius:8px;padding:18px;display:flex;justify-content:space-between;align-items:center;gap:16px}.report-banner p{color:#b7c7d0;font-size:.82rem;margin-top:5px}.report-balance{text-align:right}.report-balance span{display:block;color:#a9bdc7;font-size:.74rem}.report-balance b{display:block;color:#64d4aa;font-size:1.22rem;margin-top:4px}
  .trend-shell{background:#14242d;border-color:#29404d;color:#f5fafc;padding:18px;box-shadow:0 20px 50px rgba(20,36,45,.2)}.trend-shell .subtle{color:#9fb2bd}.trend-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-bottom:12px}.trend-kpi{background:#1b303a;border:1px solid #304953;border-radius:8px;padding:11px}.trend-kpi span{display:block;color:#9fb2bd;font-size:.7rem;font-weight:800}.trend-kpi b{display:block;font-size:.94rem;margin-top:5px}.trend-legend{display:flex;gap:14px;align-items:center;font-size:.74rem;color:#b8c7cf}.legend-dot{width:8px;height:8px;border-radius:2px;display:inline-block;margin-right:5px}.legend-dot.income{background:#4fd0a2}.legend-dot.expense{background:#ff7c82}
  .chart-wrap{height:340px;border:1px solid var(--line);border-radius:8px;background:#fff;overflow:hidden}.trend-shell .chart-wrap{background:#112029;border-color:#304953}svg{width:100%;height:100%;display:block}
  ...
  @media (max-width:1120px){.metric-grid,.mini-grid,.budget-summary{grid-template-columns:repeat(2,1fr)}.panel-grid,.panel-grid.equal{grid-template-columns:1fr}.finance-hero{grid-template-columns:1fr repeat(3,minmax(120px,1fr))}.sidebar{position:static;height:auto}.shell{grid-template-columns:1fr}.nav{grid-template-columns:repeat(4,minmax(0,1fr))}.side-meta{margin-top:0}}
  @media (max-width:720px){.content{padding:16px}.nav{grid-template-columns:repeat(2,minmax(0,1fr))}.metric-grid,.mini-grid,.budget-summary,.trend-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.metric-card .value{font-size:1.05rem;word-break:normal}.status-grid,.form-grid,.finance-hero{grid-template-columns:1fr}.hero-stat{border-left:0;border-top:1px solid #315450;padding-left:0;padding-top:10px}.form-group.full{grid-column:auto}.toolbar,.catalog-toolbar{align-items:stretch}.field,.search-field,.btn{width:100%}.table-wrap{max-height:420px}.chart-wrap{height:300px}.report-banner{align-items:flex-start;flex-direction:column}.report-balance{text-align:left}}
</style>
</head>
<body>
<div class="shell">
  ...
  <main class="content">
    ...
    <section class="view" data-view="reports" id="view-reports">
      ...
      <div class="panel-card">
        <div class="panel-head"><div><h3>Transaksi Lengkap</h3><div class="subtle" id="report-transaction-label">Periode terpilih</div></div><button class="btn" id="report-export-btn" type="button">Export CSV</button></div>
        <div class="table-wrap">
...
<script>
  ...
  function renderAnalytics(analytics) {
    ...
    $("health-score").textContent = analytics.healthScore !== undefined ? analytics.healthScore + "/100" : "-";
    ...
    $("hero-title").textContent = analytics.healthLabel ? "Kondisi keuangan: " + analytics.healthLabel : "Ringkasan keuangan terintegrasi";
    $("hero-copy").textContent = analytics.healthLabel
      ? "Skor memadukan arus kas, disiplin budget, dan konsistensi pencatatan transaksi periode ini."
      : "Data analitik akan tersedia setelah transaksi berhasil dimuat.";
  ...
  function exportCsvRows(rows, filename) {
    ...
    link.href = url;
    link.download = filename || "transaksi-dashboard.csv";
  ...
  function exportFilteredCsv() {
    exportCsvRows(filteredRecentRows(), "transaksi-dashboard.csv");
  }
  ...
  $("report-export-btn").addEventListener("click", () => exportCsvRows(currentRecent, "laporan-" + (selectedPeriod || "periode") + ".csv"));
...
• *export Mei 2026* – file CSV periode historis
