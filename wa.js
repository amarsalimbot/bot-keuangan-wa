...
function buatAnalitikDashboard(lapBulan, lapSemua, budget) {
    const budgetRows = Object.entries(budget).map(([name, limit]) => {
        const used = lapBulan.detailKategori[name] || 0;
        const percent = limit > 0 ? Math.round((used / limit) * 100) : 0;
        return { name, limit, used, percent, remaining:limit-used };
    });
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  @media (max-width:720px){.content{padding:16px}.nav{grid-template-columns:repeat(2,minmax(0,1fr))}.metric-grid,.mini-grid,.budget-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.metric-card .value{font-size:1.05rem;word-break:normal}.status-grid,.form-grid,.finance-hero{grid-template-columns:1fr}.hero-stat{border-left:0;border-top:1px solid #315450;padding-left:0;padding-top:10px}.form-group.full{grid-column:auto}.toolbar,.catalog-toolbar{align-items:stretch}.field,.search-field,.btn{width:100%}.table-wrap{max-height:420px}.chart-wrap{height:260px}}
</style>
</head>
<body>
<div class="shell">
  ...
  <main class="content">
    ...
    <section class="view" data-view="budget" id="view-budget">
      ...
      <div class="panel-grid equal">
        <div class="panel-card">
          <div class="panel-head"><div><h3>Budget Bulan Ini</h3><div class="subtle">Klik Ubah untuk menyesuaikan limit</div></div></div>
...
<script>
  ...
  function renderAnalytics(analytics) {
    ...
    $("hero-copy").textContent = analytics.healthLabel
      ? "Skor memadukan arus kas, disiplin budget, dan konsistensi pencatatan transaksi bulan ini."
  ...
  function renderBudgets(rows) {
    ...
    $("budget-list").innerHTML = rows.map(row => {
      ...
      return '<div class="budget-card"><div class="budget-card-head"><div><b>' + esc(row.name) + '</b><div class="subtle">' + esc(row.group || "Budget") + '</div></div><button type="button" class="edit-budget" data-category="' + esc(row.name) + '" data-limit="' + esc(row.limit) + '">Ubah</button></div>' +
