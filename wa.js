...
function buatJidPemilikDashboard() {
    return WHATSAPP_PHONE_NUMBER ? `${WHATSAPP_PHONE_NUMBER}@s.whatsapp.net` : "";
}

function ambilTokenDashboard(req, urlObj) {
    const headerToken = req.headers["x-dashboard-token"] || req.headers.authorization || "";
    const cleanHeader = String(headerToken).replace(/^Bearer\s+/i, "").trim();
    return cleanHeader || urlObj.searchParams.get("token") || "";
}

function aksesDashboardDiizinkan(req, urlObj) {
    if (!DASHBOARD_TOKEN) return true;
    return ambilTokenDashboard(req, urlObj) === DASHBOARD_TOKEN;
}
...
async function buatDataDashboardWeb() {
    const now = new Date().toLocaleString("id-ID",{timeZone:APP_TIMEZONE});
    const bulan = new Date().toLocaleDateString("id-ID", { timeZone: APP_TIMEZONE, month:"long", year:"numeric" });
    const ownerJid = buatJidPemilikDashboard();
    const base = {
        system: {
            ...
            port: PORT,
            owner: maskNomor(WHATSAPP_PHONE_NUMBER),
            protected: !!DASHBOARD_TOKEN
        },
        ai: {
    ...
    if (!ownerJid) {
        base.finance = {
            available: false,
            message: "WHATSAPP_PHONE_NUMBER belum diisi, jadi data keuangan belum bisa ditampilkan."
        };
        return base;
    }
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  :root{
    --bg:#f4f6f8;--surface:#ffffff;--line:#dce3ea;--text:#17212b;--muted:#667789;
    --green:#11865b;--green-soft:#e7f7ef;--red:#c0392b;--red-soft:#fdecea;
    --blue:#265d9b;--blue-soft:#e8f1fb;--amber:#b7791f;--amber-soft:#fff4d6;
    --violet:#6457a6;--shadow:0 12px 30px rgba(23,33,43,.08);
  }
  body{font-family:Inter,'Segoe UI',system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100vh}
  button,input{font:inherit}
  .shell{min-height:100vh;display:grid;grid-template-columns:248px 1fr}
  .sidebar{background:#17212b;color:#f8fafc;padding:22px 18px;display:flex;flex-direction:column;gap:22px}
  .brand{display:flex;align-items:center;gap:12px}
  .mark{width:42px;height:42px;border-radius:8px;background:#e8f1fb;color:#17212b;display:grid;place-items:center;font-weight:900}
  .brand h1{font-size:1.02rem;line-height:1.2}
  .brand small{display:block;color:#aebccc;font-size:.75rem;margin-top:3px}
  .nav{display:grid;gap:7px}
  .nav a{color:#d6dee8;text-decoration:none;padding:10px 11px;border-radius:8px;font-size:.86rem}
  ...
  .btn{border:1px solid var(--line);background:var(--surface);border-radius:8px;padding:9px 12px;cursor:pointer;color:var(--text);font-weight:700}
  .btn.primary{background:#17212b;color:white;border-color:#17212b}
  ...
  .card,.panel{background:var(--surface);border:1px solid var(--line);border-radius:8px;box-shadow:var(--shadow)}
  .card{padding:15px;min-height:112px}
  ...
  .bar{height:9px;background:#edf1f5;border-radius:8px;overflow:hidden}
  ...
  .wallet{display:flex;align-items:center;justify-content:space-between;border:1px solid var(--line);border-radius:8px;padding:10px 11px}
  .status-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
  .status-item{border:1px solid var(--line);border-radius:8px;padding:11px;background:#fafbfc}
  ...
  .cmd{border:1px solid var(--line);border-radius:8px;padding:10px;background:#fafbfc}
...
<div class="shell">
  <aside class="sidebar">
    <div class="brand"><div class="mark">WA</div><div><h1>Bot Keuangan</h1><small>Admin dashboard</small></div></div>
    <nav class="nav">
  ...
  <main class="content">
    <section class="topbar" id="overview">
      ...
      <div class="actions">
        ...
        <button class="btn" id="token-btn" type="button">Token</button>
...
<div class="lock" id="lock">
  <form class="lock-box" id="token-form">
    ...
    <p class="muted">Masukkan nilai DASHBOARD_TOKEN untuk membuka data detail.</p>
    <input id="token-input" type="password" autocomplete="current-password" placeholder="Token dashboard">
...
<script>
  ...
  let token = params.get("token") || localStorage.getItem("dashboardToken") || "";
  if (params.get("token")) history.replaceState(null, "", "/dashboard");
        ...
        try {
            ...
            if (urlObj.pathname === "/health" || urlObj.pathname === "/api/status") {
                jsonResponse(res, 200, {
                    ...
                    reconnect: jumlahReconnect,
                    dashboardProtected: !!DASHBOARD_TOKEN,
                    ai: {
            ...
            if (urlObj.pathname === "/api/dashboard") {
                if (!aksesDashboardDiizinkan(req, urlObj)) {
                    jsonResponse(res, 401, { error:"TOKEN_REQUIRED", message:"Dashboard membutuhkan DASHBOARD_TOKEN." });
                    return;
                }
                const data = await buatDataDashboardWeb();
