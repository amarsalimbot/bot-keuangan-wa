...
const BINANCE_CACHE_SECONDS     = Math.max(5, Number(process.env.BINANCE_CACHE_SECONDS || 20) || 20);
const BINANCE_RECV_WINDOW       = Math.max(1000, Number(process.env.BINANCE_RECV_WINDOW || 5000) || 5000);
const BINANCE_USDT_IDR_RATE     = Math.max(0, Number(process.env.BINANCE_USDT_IDR_RATE || 0) || 0);
...
try { serviceAccount = GOOGLE_SERVICE_ACCOUNT_JSON ? JSON.parse(GOOGLE_SERVICE_ACCOUNT_JSON) : null; }
catch { console.warn("⚠️ GOOGLE_SERVICE_ACCOUNT_JSON tidak valid JSON. Perbaiki Railway variable agar spreadsheet aktif."); }
...
async function ambilHargaBinanceUSDT() {
    const masihSegar = binancePriceCache.map && Date.now() - binancePriceCache.at < 60 * 1000;
...
function hargaAsetKeUSDT(asset, prices) {
    const a = String(asset || "").toUpperCase();
    if (["USDT", "USDC", "BUSD", "FDUSD", "TUSD", "DAI"].includes(a)) return 1;
    if (["IDR", "BIDR"].includes(a) && BINANCE_USDT_IDR_RATE > 0) return 1 / BINANCE_USDT_IDR_RATE;
    const direct = prices.get(`${a}USDT`);
    if (direct) return direct;
    const viaBtc = prices.get(`${a}BTC`) && prices.get("BTCUSDT") ? prices.get(`${a}BTC`) * prices.get("BTCUSDT") : 0;
    if (viaBtc) return viaBtc;
...
async function ambilSaldoBinance(nomor, opsi = {}) {
    ...
    const [account, prices] = await Promise.all([
        ...
        ambilHargaBinanceUSDT().catch(() => new Map())
    ]);
    const assets = (account.balances || [])
        .map(row => {
            ...
            const priceUsdt = hargaAsetKeUSDT(asset, prices);
            const valueUsdt = priceUsdt ? total * priceUsdt : null;
            return { asset, free, locked, total, priceUsdt, valueUsdt };
        })
    ...
    const totalIdr = BINANCE_USDT_IDR_RATE > 0 ? totalUsdt * BINANCE_USDT_IDR_RATE : null;
    const result = {
        enabled:true,
        configured:true,
        available:true,
        ...
        totalUsdt,
        totalIdr,
        idrRate:BINANCE_USDT_IDR_RATE || null,
...
function ringkasSaldoBinanceUntukWhatsapp(data) {
    ...
    if (data.totalIdr !== null && data.totalIdr !== undefined) teks += `\nEstimasi rupiah: *Rp ${formatRupiah(Math.round(data.totalIdr))}*`;
    teks += `\nAset aktif: *${data.assetCount}* · Update: ${data.refreshedAt}`;
    if (data.assets && data.assets.length) {
        teks += "\n\n🏦 *Top Aset:*";
        for (const row of data.assets.slice(0, 8)) {
            const nilai = row.valueUsdt !== null && row.valueUsdt !== undefined ? ` ≈ ${formatUSDT(row.valueUsdt)}` : "";
            teks += `\n• ${row.asset}: ${formatCrypto(row.total)}${nilai}`;
        }
    }
    return teks;
}
...
async function buatRingkasanBinance(jid) {
    ...
    const data = await ambilSaldoBinance(nomor, { force:true });
...
function buatAuthGoogle() {
    if (!serviceAccount?.client_email || !serviceAccount?.private_key) {
        throw new Error("GOOGLE_SERVICE_ACCOUNT_JSON belum valid atau belum lengkap.");
    }
    return new JWT({
...
async function buatDataDashboardWeb(akses, nomorDipilih = "", periodeDipilih = "") {
    ...
    const base = {
        ...
        commands: [
            ...
            { cmd:"saldo binance", desc:"Saldo Binance realtime khusus nomor 33827179200526" },
      ...
      <div class="panel-grid equal">
        <div class="panel-card binance-card">
          ...
          <div class="table-wrap">
            <table class="table"><thead><tr><th>Aset</th><th class="right">Free</th><th class="right">Locked</th><th class="right">Total</th><th class="right">Estimasi USDT</th></tr></thead><tbody id="binance-assets-body"></tbody></table>
          </div>
        </div>
        <div class="panel-card">
...
<script>
  ...
  function renderBinance(data) {
    ...
    $("binance-refresh-time").textContent = available ? "Update " + (info.refreshedAt || "-") + " · cache " + (info.cacheSeconds || 0) + " detik" : message;
    const assets = info.assets || [];
    $("binance-assets-body").innerHTML = assets.length ? assets.map(row =>
      '<tr><td><span class="asset-chip">' + esc(row.asset) + '</span></td><td class="right">' + esc(Number(row.free || 0).toLocaleString("id-ID", { maximumFractionDigits:8 })) + '</td><td class="right">' + esc(Number(row.locked || 0).toLocaleString("id-ID", { maximumFractionDigits:8 })) + '</td><td class="right"><b>' + esc(Number(row.total || 0).toLocaleString("id-ID", { maximumFractionDigits:8 })) + '</b></td><td class="right">' + (row.valueUsdt !== null && row.valueUsdt !== undefined ? esc(Number(row.valueUsdt || 0).toLocaleString("id-ID", { minimumFractionDigits:2, maximumFractionDigits:2 }) + " USDT") : "-") + '</td></tr>'
    ).join("") : '<tr><td colspan="5" class="muted">' + esc(message) + '</td></tr>';
  }
            ...
            if (urlObj.pathname === "/health" || urlObj.pathname === "/api/status") {
                jsonResponse(res, 200, {
                    ...
                    binance: {
                        ...
                        cacheSeconds: BINANCE_CACHE_SECONDS,
                        idrRateConfigured: BINANCE_USDT_IDR_RATE > 0
                    },
            ...
            if (urlObj.pathname === "/api/env-check") {
                jsonResponse(res, 200, {
                    ...
                    binance: {
                        ...
                        configured: binanceTerkonfigurasi(),
                        usdtIdrRate: BINANCE_USDT_IDR_RATE || null
                    }
                });
                return;
            }
...
Link akses ada di pesan berikutnya.`,
        ...
        if (/^(saldo\s+binance|binance|portfolio\s+binance|aset\s+binance|cek\s+binance)$/i.test(pesan)) {
