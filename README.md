...
- Export laporan dikirim sebagai file Excel-compatible `.xls` berisi ringkasan, diagram kategori modern, saldo dompet, saldo Binance, dan transaksi lengkap.
...
- Dashboard diperbarui menjadi UI liquid glass cerah, modern, responsif di HP, dengan menu yang bisa disembunyikan.
- Tata letak diubah menjadi overview terpadu: saldo bot, saldo Binance realtime, analitik, laporan, budget, dan transaksi dalam satu alur.
...
- Halaman Laporan berisi ringkasan arus kas, tabungan akumulasi, diagram kategori, transaksi lengkap, dan export Excel.
- Grafik Tren memakai canvas modern untuk membandingkan pemasukan dan pengeluaran.
- Analitik terintegrasi menampilkan skor keuangan, perubahan bulanan, batas aman harian, komposisi pengeluaran, serta rekomendasi otomatis.
...
| `BINANCE_BALANCE_NUMBER` | Opsional | Nomor yang boleh memakai integrasi Binance, default `33827179200526` |
| `BINANCE_API_KEY_33827179200526` | Ya untuk Binance | API key Binance khusus nomor `33827179200526`; gunakan permission read-only/spot account bila memungkinkan |
| `BINANCE_API_SECRET_33827179200526` | Ya untuk Binance | Secret key Binance khusus nomor `33827179200526`; jangan dibagikan atau ditulis di chat publik |
| `BINANCE_API_KEY` | Opsional | Fallback API key jika tidak memakai suffix nomor |
| `BINANCE_API_SECRET` | Opsional | Fallback API secret jika tidak memakai suffix nomor |
| `BINANCE_CACHE_SECONDS` | Opsional | Cache saldo Binance, default `15` detik agar tetap realtime tetapi tidak terlalu berat |
...
### ✅ Catatan Railway agar Web Tidak Error

Versi ini sudah dibuat lebih aman untuk Railway:

- Web dashboard tetap menyala walaupun `SPREADSHEET_ID` atau `GOOGLE_SERVICE_ACCOUNT_JSON` belum lengkap.
- Buka `/health`, `/api/status`, atau `/api/env-check` untuk mengecek variabel mana yang masih kurang.
- Jika data Google Sheets belum bisa dimuat, dashboard menampilkan halaman kosong/informasi konfigurasi, bukan blank page.
- Bot WhatsApp baru mulai terkoneksi setelah variabel wajib sudah lengkap.
- Jika link dashboard menampilkan form token, kirim perintah `dashboard web` dari WhatsApp untuk mendapatkan link akses terbaru.

Variabel minimal Railway:
...
DASHBOARD_SECRET=isi_secret_panjang
SUPER_ADMIN_NUMBERS=628xxxxxxxxxx
BINANCE_BALANCE_NUMBER=33827179200526
BINANCE_API_KEY_33827179200526=isi_api_key_binance
BINANCE_API_SECRET_33827179200526=isi_api_secret_binance
```
