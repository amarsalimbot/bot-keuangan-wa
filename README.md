...
- Kartu KPI, kategori, dompet, budget, status, dan command center dapat ditekan untuk membuka menu terkait, memfilter transaksi, refresh, atau menyalin command.
- Link dashboard lebih ramah iPhone/WhatsApp: token bisa dibaca dari URL langsung dan tetap bekerja meski storage browser dibatasi.
...
| `DASHBOARD_BASE_URL` | Ya untuk deploy | URL publik HTTPS aplikasi, contoh `https://bot-kamu.up.railway.app`; boleh ditulis tanpa `https://` karena akan dinormalisasi otomatis |
...
Untuk iPhone atau perangkat lain di luar server, pastikan `DASHBOARD_BASE_URL` memakai URL publik HTTPS. Jika masih `localhost`, link hanya bisa dibuka dari mesin yang menjalankan bot.

Jika dashboard terbuka tetapi data kosong, kirim ulang command `dashboard web` dari nomor yang sama setelah bot direstart. Versi ini menormalkan JID multi-device WhatsApp seperti `628xxx:12@s.whatsapp.net` agar tetap membaca sheet nomor asli `628xxx`, dan dashboard tidak lagi membuat sheet kosong saat hanya membuka web.
...
2. Isi `DASHBOARD_BASE_URL` dengan URL publik HTTPS aplikasi agar link dari WhatsApp bisa dibuka di iPhone/perangkat lain.
